#!/usr/bin/env bash
# Start (or restart) the persistent Clawcius agent container; the waker attaches per turn with
# `docker exec`. An existing container is reused with its writable layer and its original `docker
# run` flags; --recreate applies the current flags and discards the layer.
set -euo pipefail
...
# Every instance-specific value is overridable; the defaults are instance 1. A second instance
# must override all of them, or --recreate builds its container on instance 1's env and state.
...
# The agent's Claude home, on the state dir so --recreate does not log it out; the container is
# its only writer.
AGENT_HOME=$CLAWCIUS_STATE/agent-home
AGENT_CLAUDE=/home/agent/.claude-agent

# This script runs as npurcell, uid 1000, which is the container's `agent` uid; no chown needed.
...
# Existence and status asked separately: on a missing container `inspect -f` prints a newline
# before failing. `docker container inspect`, never bare `inspect`, which would match the image.
...
# Report the container's config read back off the container, in the fields that differ between
# deployments. Printed to stdout, which is the unit's journal, readable without sudo: never print
# .Config.Env, .Config.Cmd or .Mounts here.
describe_container() {
  # `local` on its own line: `local raw=$(cmd)` takes local's exit status.
  local raw created id image imageid runtime init memory pids
  # Guarded: this runs under `set -e` from systemd, and a readback must not take the unit down.
...
# A reused container keeps the flags it was created with; say so, then report what it has.
warn_flags_inert() {
...
# clawcius-internal has no gateway out; Squid is the only route, default-allow with a blocklist.
...
# --init: tini as pid 1 reaps orphans against --pids-limit. To restore a snapshot:
#     CLAWCIUS_IMAGE=<repo>:snap-<stamp> docker/run-container.sh --recreate
# Not --cap-drop=ALL: root execs here run apt, and dpkg needs CHOWN, SETUID/SETGID, DAC_OVERRIDE, FOWNER.
docker run -d \
  --name "$NAME" \
  --runtime=runsc \
...
# Never mount the docker socket in here: a privileged container would make gVisor decorative.
