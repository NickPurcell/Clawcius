...
function describeSpawn(agentId: string): string {
  return [
    ...
    'OTHERWISE WONDER. It keeps one message current — a task list of at most four',
    'items, edited in place — so the work is visible without you narrating it. Spawn',
    'one, then DM it the list; it learns what to track from that DM and from nothing',
    'else, so a spawn without one leaves it idle and correct and useless. One per',
    'crew is enough, and it is the cheapest agent you can have: it runs on a faster',
    'model than you do. For work that finishes in a turn or two, do not bother — the',
    'list would be stale before anyone read it.',
    '',
