...
What it changes is convenience, not access. The board is a SQLite file on the
operator's own disk; anyone who can read the page can already read the file,
and `sqlite3 /var/lib/hamachi/hamachi.db 'select * from mail'` was always the
alternative. What it does not change: the page is loopback-bound and read-only,
