...
the session has no shell at all — but **the deployed executor runs with dry run
off**, set by `OPS_DRY_RUN` on an `Environment=` line in
`systemd/clawcius-ops.service` rather than in the tracked config, which keeps
the safe default. The boot line in `journalctl -u clawcius-ops` names the value
in force and which of the two decided it.
...
| `systemd/clawcius-ops.service` | `Environment=OPS_DRY_RUN=false` — the one setting that legitimately differs between this repository and this machine, and the reason it is here rather than as an uncommitted edit to the file above | yes; installing it is an operator step |
